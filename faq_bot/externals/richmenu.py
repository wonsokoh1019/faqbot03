...
def make_add_rich_menu_body(rich_menu_name):
    ...
    action1 = make_postback_action("query",
                                   display_text="Do you have any "
                                                "work-related questions? \n"
                                                "\"Find FAQ\" by task",
                                   label="\"Find FAQ\" by task",)

    bound2 = make_bound(0, 1160, 1250, 526)
    action2 = make_postback_action("enquire",
                                   display_text="Ask a person in charge",
                                   label="Ask ... charge")

    bound3 = make_bound(1250, 1160, 1250, 526)
    action3 = make_postback_action("to_first",
                                   display_text="Go to Initial Menu",
                                   label="Go to Initial Menu")
...
def init_rich_menu():
    ...
    :return: rich menu id
    """
    rich_menu_id = get_rich_menus(RICH_MENUS["name"])
    if rich_menu_id is None:
